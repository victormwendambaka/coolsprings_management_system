...
<?php 
    ...
    <h4 style="margin-right: 5px; background-color: green; padding: 5px; display: inline-block;"><a href="payment.php" style="text-decoration: none; color: white;">Confirmed Payments</a></h4>
    <span style="color: white;">|</span>
    <h4 style="margin-left: 5px; background-color: red; padding: 5px; display: inline-block;"><a href="cancelledpayment.php" style="text-decoration: none; color: white;">Cancelled Payments</a></h4>
</div>
...
if ($query->rowCount() > 0) {
    foreach ($results as $result) {
        if ($result->status == 2) { // Check if status is 1 (you can adjust the status value if needed)
            $dd = $result->totaldays;
            $rm = $result->rooms;
            $pr = $result->price;
            $tt = $dd * $pr * $rm;
